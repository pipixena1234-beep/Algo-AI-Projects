...
document.addEventListener('click', e => {
  ...
  // --- SPRITE IMAGE ---
// --- Create the Wrapper (Handles the Drop) ---
const wrapper = document.createElement('div');
wrapper.className = 'drop-wrapper is-falling';
wrapper.style.left = (e.clientX - 50) + 'px';
wrapper.style.top = e.clientY + 'px';

// --- Create the Sprite (Handles the Eating) ---
const spritePic = document.createElement('div');
spritePic.className = 'sprite-inner';
spritePic.style.backgroundImage = `url('chibi1_eat.png')`;

wrapper.appendChild(spritePic);
document.body.appendChild(wrapper);

// Clean up after everything is done (Drop 2s + Eat 3s)
setTimeout(() => {
  wrapper.style.transition = 'opacity 1s ease';
  wrapper.style.opacity = '0';
  setTimeout(() => wrapper.remove(),  3000 + Math.random() * 2000);
